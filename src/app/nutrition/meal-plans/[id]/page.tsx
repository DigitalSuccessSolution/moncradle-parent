...
export default function MealDetailPage() {
  ...
  const handleAddToCart = async (e: React.MouseEvent) => {
    ...
    try {
      await dispatch(addToCartAsync({ itemId: mealId, itemType: "meal" })).unwrap();
      ...
      <main className="max-w-[1200px] mx-auto px-4 md:px-8 pt-4 pb-24 md:py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 mt-0 md:mt-6">
          ...
          >
            ...
            >
              {images.length > 0 ? images.map((src: string, idx: number) => (
                ...
                <div className="w-full h-full flex-shrink-0 snap-center relative bg-[#F8FAFC] flex flex-col items-center justify-center text-gray-400">
                  <ShoppingCart className="w-12 h-12 mb-2 opacity-50" />
                  <span className="font-semibold text-xs">No Image Available</span>
                </div>
              )}
            </div>
          ...
          >
            ...
            <div className="flex justify-between items-end mb-4 relative">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-extrabold text-gray-800">4.9</span>
              </div>
            </div>
            ...
            <div className="mb-8 space-y-3">
              {meal.category && (
                <div className="flex items-start">
                  ...
                  <span className="text-sm font-semibold text-gray-900 capitalize">{meal.category}</span>
                </div>
              )}
              {meal.suitableForAgeGroup && (
                <div className="flex items-start">
                  <span className="w-28 text-sm text-gray-500 font-medium">Age Group</span>
                  <span className="text-sm font-semibold text-gray-900">{meal.suitableForAgeGroup}</span>
                </div>
              )}
            </div>
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="bg-orange-50 p-3 rounded-xl border border-orange-100">
                    <p className="text-[10px] font-semibold text-orange-600 uppercase tracking-wider mb-1">Calories</p>
                    <p className="font-black text-gray-900 text-sm">{meal.nutritionalInfo.calories} kcal</p>
                  </div>
                  <div className="bg-blue-50 p-3 rounded-xl border border-blue-100">
                    <p className="text-[10px] font-semibold text-blue-600 uppercase tracking-wider mb-1">Protein</p>
                    <p className="font-black text-gray-900 text-sm">{meal.nutritionalInfo.protein}g</p>
                  </div>
                  <div className="bg-amber-50 p-3 rounded-xl border border-amber-100">
                    <p className="text-[10px] font-semibold text-amber-600 uppercase tracking-wider mb-1">Carbs</p>
                    <p className="font-black text-gray-900 text-sm">{meal.nutritionalInfo.carbs}g</p>
                  </div>
                  <div className="bg-emerald-50 p-3 rounded-xl border border-emerald-100">
                    <p className="text-[10px] font-semibold text-emerald-600 uppercase tracking-wider mb-1">Fat</p>
                    <p className="font-black text-gray-900 text-sm">{meal.nutritionalInfo.fat}g</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {/* Action Bar */}
            <div className="mt-auto bg-white p-3 lg:p-0 border-t border-gray-100 lg:border-none fixed bottom-0 left-0 w-full lg:relative lg:bottom-0 z-40 flex gap-3 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] lg:shadow-none pb-safe">
              
              {/* Add to Plan Button */}
              <button
                onClick={() => setShowPlanModal(true)}
                className="flex-1 h-12 md:h-14 bg-white border border-[var(--color-primary)] text-[var(--color-primary)] font-semibold text-sm md:text-base rounded-lg hover:bg-[var(--color-primary)]/5 transition-all duration-300 flex items-center justify-center gap-2 group/plan shadow-sm"
              >
              ...
              {meal.inStock ? (
                cartQuantity > 0 ? (
                  <div 
                    ...
                    className="flex-1 h-12 md:h-14 bg-[var(--color-primary)] text-white font-semibold rounded-lg shadow-sm flex items-center justify-center cursor-pointer hover:bg-[#527d89] transition-colors"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 h-12 md:h-14 bg-[var(--color-primary)] text-white font-semibold text-sm md:text-base rounded-lg hover:bg-[#527d89] hover:shadow-md transition-all duration-300 flex items-center justify-center gap-2 group/btn shadow-sm"
                  >
                ...
                <button
                  disabled
                  className="flex-1 h-12 md:h-14 bg-gray-300 text-gray-500 font-semibold text-sm md:text-base rounded-lg shadow-sm flex items-center justify-center gap-2 cursor-not-allowed"
                >
                  Out of Stock
                </button>
              )}
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="p-5 border-b border-gray-100 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold text-gray-900">Select Day</h3>
