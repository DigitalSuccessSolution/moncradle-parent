...
export function ReportsTab({ records, setSelectedRecord, formatDate }: ReportsTabProps) {
  if (records.length === 0) {
    return (
      <div className="text-center py-20 bg-white rounded-lg border border-gray-100">
         ...
         <p className="text-xs text-gray-400 mt-1">Upload your baby's reports using the button above.</p>
      </div>
    );
  }
      ...
      {records.map((record, i) => (
        <div key={record._id || i} className="bg-white rounded-lg p-5 border border-gray-100 shadow-sm hover:shadow-md hover:border-gray-200 transition-all flex flex-col h-full cursor-pointer">
          <div className="flex justify-between items-start mb-3">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-lg overflow-hidden border border-gray-100 bg-gray-50 flex-shrink-0 flex items-center justify-center">
                {record.babyId?.parentId?.avatar ? (
                  <img src={record.babyId.parentId.avatar} alt="Parent" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-purple-500 font-bold text-lg">P</span>
                )}
              </div>
              <div>
                <h3 className="font-semibold text-gray-900">{record.babyId?.parentId?.name ? `${record.babyId.parentId.name} (Parent)` : "Self Upload"}</h3>
                <p className="text-[11px] font-medium text-gray-500">Health Report</p>
              </div>
            </div>
          ...
          <div className="mb-5 flex-1">
            <span className="inline-block px-2 py-0.5 text-[10px] font-semibold rounded uppercase tracking-wider mb-2 border bg-purple-50 text-purple-600 border-purple-100">
              My Report
            </span>
