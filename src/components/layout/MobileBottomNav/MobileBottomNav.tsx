...
export function MobileBottomNav({ isHidden = false }: { isHidden?: boolean }) {
  ...
  return (
    <nav 
      className={`md:hidden fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-md border-t border-[var(--color-border)] px-4 py-2.5 flex justify-between items-center z-50 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] transition-transform duration-300 ease-in-out ${isHidden ? 'translate-y-full' : 'translate-y-0'}`}
    >
      {navItems.map((item) => {
        ...
        return (
          <Link 
            key={item.name} 
            href={item.href} 
            className={`flex flex-col items-center gap-1 transition-colors cursor-pointer ${isActive ? "text-[var(--color-primary)]" : "text-gray-600 hover:text-gray-900"}`}
          >
